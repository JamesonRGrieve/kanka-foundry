...
function getNestedValue<T, P extends Path<T>>(obj: T, path: P): PathValue<T, P> {
    const result: unknown = path.split('.').reduce<unknown>((acc, part) => {
        return acc && typeof acc === 'object' ? Reflect.get(acc, part) : undefined;
    }, obj);
    assertType<PathValue<T, P>>(result);
    return result;
}

export default function groupBy<Type, P extends Path<Type> = Path<Type>>(array: Type[], key: P): Map<PathValue<Type, P>, Type[]> {
