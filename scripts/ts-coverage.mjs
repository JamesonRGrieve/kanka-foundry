 ...
 *
 * Counts these patterns across `src/**\/*.ts`:
 *   - bare `: any` annotations
 *   - ` as any` casts
 *   - `@ts-expect-error` directives
 *   - `@ts-ignore` directives
 *
 * Aggregates by top-level directory under `src/` (api/, apps/, util/, ...).
 * Outputs:
...
function* walk(dir) {
    for (const name of readdirSync(dir)) {
        ...
        else if (st.isFile() && name.endsWith('.ts') && !name.endsWith('.d.ts') && !name.endsWith('.test.ts') && !name.endsWith('.spec.ts')) yield full;
    }
}
