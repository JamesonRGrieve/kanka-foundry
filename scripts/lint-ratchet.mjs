#!/usr/bin/env node
/**
 * ESLint warning ratchet. Runs eslint on src/ and compares warning count to
 * .eslint-warning-baseline. Fails when the count goes UP, succeeds when it
 * goes down or stays the same. Errors are never allowed.
 *
 * Usage:
...
let lintOutput;
const lintOutputPath = resolve(tmpdir(), `kanka-eslint-${process.pid}.json`);
try {
    execSync(`/bin/bash -lc './node_modules/.bin/eslint src/ --ext .ts --format json > "${lintOutputPath}"'`, {
        encoding: 'utf8',
...
if (existsSync(lintOutputPath)) unlinkSync(lintOutputPath);
